...
* [实验过程](#02)
  ...
  * [Part 3 开启AP功能](#023)
  * [Part 4 使用手机连接不同配置状态下的AP对比实验](#024)
  * [Part 5 找到OpenWrt配置并截图](#025)
* [问题和解决](#04)
* [参考资料](#06)
...
* 配置`ssh`远程桌面连接
  ...
  参考上学期的实验7的实验报告配置：（上学期实验报告有小错误，已经修改）
...
* Encryption:None
  ...
  手机WiFi直接连接上，`iPad`会提示不安全。

* Encryption:WPA2 PSK(CCMP)

  ![WPA2](img/WPA2加密模式.png)

  需要输入密码，并且每次点击`Save&Apply`，会自动断开连接。

### <span id = "025">Part 5</span> 找到`OpenWrt`配置并截图
...
- [x] **Q6：无线网络一直是`not associated`**

  A6：

  * 因为一直设置不好信号（虽然上课的时候解决了，但是到图书馆后又失败），所以手机连接实验一直没做好。
  * 一步一步重新操作后，终于成功。并且发现自己之前在安装`hostapd` 和`wpa-supplicant`之后忘记重启。感觉是这个问题。
  * 还有就是可能图书馆网络不好。

- [ ] **Q7：`iw dev`没有输出结果**

  A7：有时候有结果，下一秒再执行一次可能就没有结果。不清楚为什么。但是后来配置好了以后也没有出现这种现象。可能也是自己配置的时候出了问题。
